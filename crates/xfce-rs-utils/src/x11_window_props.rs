...
/// Set plugin window properties to make it appear as a utility window and skip taskbar
/// This is used for plugin windows that are embedded in the panel
pub fn set_plugin_window_properties(window_name: &str) -> Result<()> {
    ...
    let window_id = find_window_by_name(&conn, root_window, window_name, &atoms)
        .context("Failed to find plugin window")?;
    
    info!("Found plugin window: {}, setting properties", window_id);
    
    // Set window type to UTILITY (not DOCK - that's for panels)
    ...
    info!("Successfully set plugin window properties");
    Ok(())
}
